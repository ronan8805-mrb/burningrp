...
function signingSecret(): string {
  ...
  if (set) return set;
  const vercel = env("VERCEL_DEPLOYMENT_ID") ?? env("VERCEL_GIT_COMMIT_SHA");
  if (vercel) return (vercel + vercel + vercel).slice(0, 64);
...
export const auth = betterAuth({
  ...
  // On Vercel, fall back to the deployment id so every isolate signs OAuth
  // state with the same key (a per-process random secret made Google/X bounce).
